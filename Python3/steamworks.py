...
class Steam:
	...
	def Init():
		...
		if sys.platform == 'linux' or sys.platform == 'linux2':
			Steam.cdll = CDLL(os.path.join(os.getcwd(), "Steamworks.so"))
			print("INFO: SteamworksPy loaded for Linux")
			Steam.loaded = True
		# Loading SteamworksPy API for Mac
		elif sys.platform == 'darwin':
			Steam.cdll = CDLL(os.path.join(os.getcwd(), "Steamworks.dylib" ))
		...
		# Loading SteamworksPy API for Windows
			...
			if OS_BIT == '32bits':
				Steam.cdll = CDLL(os.path.join(os.getcwd(), "Steamworks.dll"))
			else:
				Steam.cdll = CDLL(os.path.join(os.getcwd(), "Steamworks64.dll"))
...
class SteamUserStats:
	...
	def GetAchievement(name):
		if Steam.isSteamLoaded():
			return Steam.cdll.GetAchievement(name.encode('ascii'))
		else:
			return ""
	...
	def GetStatFloat(name):
		if Steam.isSteamLoaded():
			return Steam.cdll.GetStatFloat(name.encode('ascii'))
		else:
			return 0
	...
	def GetStatInt(name):
		if Steam.isSteamLoaded():
			return Steam.cdll.GetStatInt(name.encode('ascii'))
		else:
			return 0
	...
	def SetAchievement(name):
		if Steam.isSteamLoaded():
			return Steam.cdll.SetAchievement(name.encode('ascii'))
		else:
			return False
	# Set a statistic
	@staticmethod
	def SetStat(name, value):
		if Steam.isSteamLoaded():
			if isinstance(value, float):
				return Steam.cdll.SetStatFloat(name.encode('ascii'), value)
			elif isinstance(value, int):
				return Steam.cdll.SetStatInt(name.encode('ascii'), value)
			else:
	...
	def ClearAchievement(name):
		if Steam.isSteamLoaded():
			return Steam.cdll.ClearAchievement(name.encode('ascii'))
		else:
			return False
#------------------------------------------------
